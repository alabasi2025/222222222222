    ...
    if (type === "in" || type === "return") {
      ...
      if (req.body.referenceType === "purchase") {
        ...
        if (calculatedTotalCost > 0) {
          // Extract supplier account ID - first try from supplier record in DB, then from notes
          let supplierAccountId: string | null = null;
          if (req.body.notes) {
            // Try to get supplier ID from notes and look up their account from suppliers table
            const supplierIdMatch = req.body.notes.match(/supplierId:([^\s]+)/);
            if (supplierIdMatch) {
            ...
            // Fallback: try supplierAccountId from notes
          ...
          if (item.length > 0 && item[0].accountId && supplierAccountId) {
            ...
            if (supplierAccount.length > 0) {
              ...
              // Debit increases asset accounts (stock)
              await db
                .update(accounts)
                .set({
              ...
              // Credit increases liability accounts (supplier)
              await db
                .update(accounts)
                .set({
                  balance: sql`${accounts.balance} + ${calculatedTotalCost}`,
          ...
          if (
            ...
            try {
              ...
              const exchangeAccount = await db
                .select()
                .from(banksWallets)
      ...
      if (toAccountId && calculatedTotalCost > 0) {
        ...
        if (item.length > 0) {
          ...
          if (stockAccountId && cogsAccountId) {
            ...
            // Update account balances
            // Debit increases expense accounts (COGS/consumption)
            await db
              .update(accounts)
              .set({
                balance: sql`${accounts.balance} + ${calculatedTotalCost}`,
            ...
            // Credit decreases asset accounts (stock)
            await db
              .update(accounts)
              .set({
                balance: sql`${accounts.balance} - ${calculatedTotalCost}`,
