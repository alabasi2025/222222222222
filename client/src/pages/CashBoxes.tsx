...
export default function CashBoxes() {
  ...
  const loadData = async () => {
    try {
      ...
      const normalizedBoxes = boxesData.map((box: any) => ({
        ...
        currencies:
          box.currencies ||
          (box.currency ? [box.currency] : ["YER", "SAR", "USD"]),
  ...
  const allCashAccounts = accounts.filter(
    account =>
  ...
  const handleAddBox = async () => {
    ...
    try {
      const box = {
        ...
        type: newBox.type,
        accountId:
          newBox.accountId && newBox.accountId !== "none"
            ? newBox.accountId
            : null,
        branchId:
          newBox.branchId && newBox.branchId !== "main"
            ? newBox.branchId
            : null,
      ...
      toast.success(
        `تم إضافة الصندوق بنجاح (${newBox.currencies.length} عملة)`
      );
      await loadData();
  ...
  const handleEditBox = async () => {
    ...
    const currencies =
      editingBox.currencies ||
      (editingBox.currency ? [editingBox.currency] : ["YER", "SAR", "USD"]);
    ...
    try {
      ...
      const boxData = {
        ...
        accountId:
          editingBox.accountId && editingBox.accountId !== "none"
            ? editingBox.accountId
            : null,
        branchId:
          editingBox.branchId && editingBox.branchId !== "main"
            ? editingBox.branchId
            : null,
        responsible:
          editingBox.responsiblePerson || editingBox.responsible || null,
        isActive:
          editingBox.status === "active"
            ? true
            : editingBox.isActive !== undefined
              ? editingBox.isActive
              : true,
      };
      ...
      toast.success(
        `تم تحديث بيانات الصندوق بنجاح (${currencies.length} عملة)`
      );
      await loadData();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            إدارة الصناديق النقدية والعهد لـ{" "}
            <span className="font-bold text-primary">{currentEntity.name}</span>
          </p>
        </div>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Select
                    ...
                    onValueChange={v =>
                      setNewBox({ ...newBox, accountId: v === "none" ? "" : v })
                    }
                  >
                    <SelectTrigger className="col-span-3">
                    ...
                    {(["YER", "SAR", "USD"] as const).map(currency => {
                      ...
                      return (
                        ...
                        >
                            ...
                            onCheckedChange={checked => {
                              if (checked) {
                                setNewBox(prev => ({
                                  ...
                                  defaultCurrency:
                                    prev.currencies.length === 0
                                      ? currency
                                      : prev.defaultCurrency,
                                }));
                              } else {
                                setNewBox(prev => {
                                  const newCurrencies = prev.currencies.filter(
                                    c => c !== currency
                                  );
                                  return {
                                    ...prev,
                          ...
                          />
                          <label
                            htmlFor={`new-${currency.toLowerCase()}`}
                    ...
                    {newBox.currencies.length > 0 && (
                      <div className="mt-2">
                        ...
                        <Select
                          ...
                          onValueChange={v =>
                            setNewBox({ ...newBox, defaultCurrency: v })
                          }
                        >
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-start gap-4">
                  ...
                  <div className="col-span-3 space-y-2">
                    {(["YER", "SAR", "USD"] as const).map(currency => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          <label
                            htmlFor={`edit-${currency.toLowerCase()}`}
                            className="text-sm font-medium cursor-pointer"
                          >
                            {currencyLabels[currency]}
                          </label>
                    ...
                    {editingBox &&
                      (editingBox.currencies || editingBox.currency) && (
                        <div className="mt-2">
                          <Label className="text-sm text-muted-foreground">
                            العملة الافتراضية:
                          </Label>
                          <Select
                            value={
                              editingBox.defaultCurrency ||
                              editingBox.currency ||
                              "YER"
                            }
                            onValueChange={v =>
                              setEditingBox((prev: any) =>
                                prev ? { ...prev, defaultCurrency: v } : null
                              )
                            }
                          >
                            <SelectTrigger className="mt-1">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">0.00 ر.ي</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-emerald-600">0.00 ر.ي</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-rose-600">0.00 ر.ي</div>
            <p className="text-xs text-muted-foreground mt-1">0 عمليات صرف</p>
          </CardContent>
        </Card>
      </div>
          ...
          <TableBody>
            {visibleCashBoxes.length === 0 ? (
              <TableRow>
                ...
                >
                  لا توجد صناديق مسجلة لـ {currentEntity.name}. قم بإضافة صندوق
                  جديد.
                </TableCell>
              </TableRow>
            ) : (
                      ...
                      <div className="flex gap-1 flex-wrap">
                        {currencies.map((currency: string) => (
                          <Badge
                            ...
                            variant={
                              currency ===
                              (box.defaultCurrency || box.currency || "YER")
                                ? "default"
                                : "outline"
                            }
                            className="text-xs"
