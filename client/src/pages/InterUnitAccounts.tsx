...
export default function InterUnitAccounts() {
  ...
  const [_interUnitAccounts, setInterUnitAccounts] = useState<
    InterUnitAccount[]
  >([]);
  ...
  const loadData = async () => {
    ...
    try {
      const [entitiesRes, accountsRes, interAccountsRes] = await Promise.all([
        ...
        api.get<InterUnitAccount[]>(
          "/inter-unit-transfers/inter-unit-accounts"
        ),
      ]);
  ...
  const _getNetBalance = (entityId: string, relatedEntityId: string) => {
    const account1 = getIntercompanyAccounts().find(
      acc =>
        ...
        acc.name.includes(getEntityName(relatedEntityId))
    );
    const account2 = getIntercompanyAccounts().find(
      acc =>
        acc.entityId === relatedEntityId &&
        acc.name.includes(getEntityName(entityId))
    );
  ...
  const handleReconcile = async () => {
    if (
      !reconcileData.fromEntityId ||
      !reconcileData.toEntityId ||
      !reconcileData.amount
    ) {
      toast({
        title: "خطأ",
  ...
  const accountsByEntity = entities.reduce(
    (acc, entity) => {
      acc[entity.id] = intercompanyAccounts.filter(
        a => a.entityId === entity.id
      );
      return acc;
    },
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${Math.abs(totalDebit - totalCredit) < 0.01 ? "text-green-500" : "text-yellow-500"}`}
                >
        ...
        return (
          <Card key={entity.id}>
            <CardHeader>
              ...
              <CardDescription>حسابات الجاري مع الوحدات الأخرى</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  ...
                  {entityAccounts.map(account => (
                    <TableRow key={account.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            account.type === "asset" ? "default" : "secondary"
                          }
                        >
                          {account.type === "asset"
                            ? "مدين (أصل)"
                      ...
                      <TableCell
                        className={`font-semibold ${parseFloat(account.balance) > 0 ? "text-green-600" : parseFloat(account.balance) < 0 ? "text-red-600" : ""}`}
                      >
                        {parseFloat(account.balance).toLocaleString()} ر.ي
                      </TableCell>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>قم بتعديل اسم حساب الجاري</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={value =>
                  setReconcileData({ ...reconcileData, fromEntityId: value })
                }
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={value =>
                  setReconcileData({ ...reconcileData, toEntityId: value })
                }
              >
                <SelectTrigger>
                  <SelectValue placeholder="اختر الوحدة" />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e =>
                  setReconcileData({ ...reconcileData, amount: e.target.value })
                }
                placeholder="0.00"
              />
            </div>
